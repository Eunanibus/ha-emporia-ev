...
| `sensor`        | **Power**                | `sensor.<name>_power`              | Charging power in **W**. This is the primary energy-monitoring signal. Derived from the Emporia 1-minute energy bucket (kWh x 60 x 1 000), so it is an _average_ over the last ~1 minute, not a true instantaneous reading. |
...
A hub added with an email and password re-authenticates silently when its refresh token expires.
The integration performs a full login using the stored credentials and continues without any prompt.
